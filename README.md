...
```console
docker run cisagov/example:0.2.0+build.1
```
...
1. Create a `compose.yml` file similar to the one below to use [Docker Compose](https://docs.docker.com/compose/).
    ...
    services:
      example:
        ...
        image: cisagov/example:0.2.0+build.1
        ports:
          - protocol: tcp
            published: "8080"
            target: 8080
...
1. Pull the new image:
    ...
    ```console
    docker pull cisagov/example:0.2.0+build.1
    ```
...
|-----------|-------------|
|`cisagov/example:0.2.0+build.1`| An exact release version. |
...
```console
docker build \
  --tag cisagov/example:0.2.0+build.1 \
